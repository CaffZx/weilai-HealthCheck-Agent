...
2. 现象即原因型：本文件里检测命中条件
3. 表现型：本文件不检测命中，由运行时适配器触发 R3 判定
...
# -----------------------------------------------------------------------------
def _stockout_severity(变体重要性: str | None, 原默认: str) -> str:
    """重点变体缺货分级严重度：主要色 S0 / 次要色 S1 / 长尾色 S2。

    规则（用户确认 2026-08-26）：
      - 主要色缺货 → S0
      - 次要色缺货 → 默认 S1（销量贡献高/广告承接/影响核心尺码完整性时可上调 S0，待子体销量数据支持）
      - 长尾色缺货 → 默认 S2
      - 未知/未标注 → 保持原默认（现状不变）
    """
    if 变体重要性 == "主要色":
...
def detect_FBA可售库存为0(
    ...
    if FBA可售库存 == 0:
        ...
        return 命中异常(
            ...
            触发字段={"FBA可售库存": 0},
            默认严重度=_stockout_severity(
                变体重要性, 查默认严重度("FBA可售库存为0", r3)
            ),
        )
    return None
...
def detect_标题异常(
    ...
    """§2.2 标题异常（有子体身份时按子 ASIN 记录）。"""
    if 标题字段 is None and 审核状态 is None and not 子体ASIN:
...
def detect_五点异常(
    ...
    """§2.2 五点异常（子 ASIN 级）。数据源标注口径：前台(child_detail.features)/ERP(product_info)。"""
    ...
    r2 = r2 or 加载R2()
    最低 = r2["五点异常"]["最低五点数量"]
    # 五点内容项数为 0 意味着数据源根本没返回有效内容（前台 features 为空 / ERP
    # fiveBulletPoint1-5 全部为空），而非 Listing 真的没有五点。此时不应报“五点异常”，
    # 而应标记为数据不足，避免与运营在亚马逊前台看到的真实五点产生矛盾。
    if 五点内容项数 == 0:
        return 观察类结果(
            问题点位="五点异常",
            观察类型=观察_数据不足,
            原因=f"{数据源}未返回该子体的五点描述内容（项数为 0），"
                 "可能为数据未同步，非 Listing 本身缺陷",
            缺失字段=["五点内容项数"],
            上下文={
                "子体ASIN": 子体ASIN,
                "子体SKU": 子体SKU,
    ...
    if 五点内容项数 is not None and 五点内容项数 < 最低:
        异常原因.append(f"五点项数 {五点内容项数} < {最低}（{数据源}口径）")
        触发["五点内容项数"] = 五点内容项数
        触发["五点数据源"] = 数据源
    if 审核状态 in ("未通过", "异常"):
        异常原因.append(f"审核状态={审核状态}")
...
def detect_促销异常(
    ...
    if ERP活动配置存在 and 前台促销展示 == "未展示":
        ...
        触发["ERP配置"] = True
        触发["前台展示"] = 前台促销展示
    # 反向判定：ERP 明确未配置活动（is False，不含 None=数据缺失），
    # 但前台出现促销（划线价/省幅/优惠券）→ 疑似未登记的促销或平台自动促销。
    if ERP活动配置存在 is False and 前台促销展示 == "已展示":
        异常原因.append("ERP未配置活动但前台存在促销（疑似未登记/平台自动促销）")
        触发["ERP配置"] = False
        触发["前台展示"] = 前台促销展示
    if 活动审核状态 in ("未通过", "异常"):
...
def detect_近期集中差评(
    近期低星评论数: int | None,
    同类问题: str | None = None,
    同类问题占比: float | None = None,
    现有评分数: int | None = None,
    触发条件: str | None = None,
    评论样本: list[dict] | None = None,
    r3: dict | None = None,
) -> 命中异常 | 观察类结果 | None:
    """§2.7 近期集中差评：上游负责评论窗口、去重和问题归类。"""
    if 近期低星评论数 is None:
        return 观察类结果(
            问题点位="近期集中差评", 原因="缺失近 14 天评论证据",
            缺失字段=["14天新增评论及星级", "评论文本"],
        )
    if not 触发条件:
        return None
    r3 = r3 or 加载R3()
    details = [f"近14天低星评论 {近期低星评论数} 条", 触发条件]
    if 同类问题:
        details.append(f"主要集中于{同类问题}（{(同类问题占比 or 0):.0%}）")
    return 命中异常(
        问题点位="近期集中差评",
        作用层级="链接级",
        类型="表现型",
