...
function productRequiresAction(product){
  ...
  return (product?.events || []).some(event => {
    ...
    if (lifecycleStatus === '待观察' && event.last_action === '自动恢复观察') return false;
...
function isEventHandled(event){
  return Boolean(event?.active_observation) || ['已完成', '已关闭'].includes(event?.status);
}

function formatIsoSlashMonthDay(iso){
  const parts = String(iso || '').slice(0, 10).split('-');
  return parts.length === 3 ? `${parts[1]}/${parts[2]}` : '-';
}

function eventStatusLabel(event){
  if (event?.internal_status === '待观察' && event?.last_action === '自动恢复观察') return '疑似已恢复';
  if (!event?.active_observation) return isEventHandled(event) ? '已完成' : '未完成';
  const date = formatIsoSlashMonthDay(event.observation_at);
  if (event.observation_phase === '待运营确认') return `待确认 · ${date}已有结论`;
  return `观察中 · ${date}确定效果`;
}

function productIssueTagsHtml(events){
  const severityOrder = {S0: 0, S1: 1, S2: 2};
  return issueDisplayRows(events)
    .sort((left, right) =>
    ...
    .map(event => {
      const handled = event.grouped
        ? (event.grouped_events || []).every(isEventHandled)
        : isEventHandled(event);
      const title = event.grouped
        ? `${event.issue || event.category || '异常'} · ${(event.variants || []).length} 个子 ASIN`
        : event.issue || event.category || '异常';
      return `<span class="product-issue-tag ${severityClass(event)} ${handled ? 'resolved' : ''}">${esc(eventSeverity(event))} · ${esc(title)}</span>`;
    }).join('');
}
...
function assignBadgeHtml(product){
  ...
  const label = product.assigned_to_me ? '协作指派给我' : '协作指派给';
  return `<span class="${cls}" title="协作指派给 ${names}">👤 ${label} ${names}</span>`;
}
function activeErpOwners(product){
  return (product.erp_owners || []).filter(owner =>
    Number(owner.user_state) === 1 && String(owner.user_name || '').trim());
}
function erpOwnerBadgeHtml(product){
  const owners = activeErpOwners(product);
  if (!owners.length) return '<span class="assign-badge">ERP 负责人待修正</span>';
  const names = owners.map(owner => esc(owner.user_name)).join('、');
  return `<span class="assign-badge" title="ERP 实际负责人：${names}">ERP 负责人：${names}</span>`;
}
function productTaskCardHtml(product){
  const mainEvent = product.events[0] || product;
  ...
  return `
    ...
    <div class="product-card-copy"><b>${esc(mainEvent.product_name || mainEvent.parent_asin)}</b><span class="product-card-meta"><span class="product-card-identity">${esc(mainEvent.parent_asin)} · ${esc(mainEvent.shop_account || '-')}</span>${product.amazon_url ? `<a class="product-amazon-link" href="${esc(product.amazon_url)}" target="_blank" rel="noopener noreferrer" title="打开 Amazon 前台商品页" aria-label="打开 Amazon 前台商品页">Amazon前台 <i aria-hidden="true">&#8599;</i></a>` : ''}</span><div class="product-issue-tags">${product.events.length ? productIssueTagsHtml(product.events) : '<span class="product-issue-tag resolved">本次巡检无异常</span>'}</div><div class="product-issue-tags">${erpOwnerBadgeHtml(product)}${assignBadgeHtml(product)}</div></div>
    <div class="product-card-badges"><i class="priority ${String(product.priority || mainEvent.priority || 'P2').toLowerCase()}">${esc(product.priority || mainEvent.priority || '-')}</i><span>${formatScore(product.product_score ?? mainEvent.score)} 分</span><b class="status ${statusClass(product.product_status)}">${esc(product.product_status)}</b></div>`;
}

function issueDisplayRows(events){
  ...
  return [...groups.entries()].map(([issue, groupedEvents]) => {
    const currentEvents = groupedEvents.filter(event => !['已完成', '已关闭'].includes(event.status));
    const displayedEvents = currentEvents.length ? currentEvents : groupedEvents;
    if (displayedEvents.length === 1) return displayedEvents[0];
    const representative = [...displayedEvents].sort((left, right) =>
      Number(right.score || 0) - Number(left.score || 0) || Number(right.days || 0) - Number(left.days || 0)
    )[0];
    return {
      ...representative,
      issue,
      grouped: true,
      grouped_events: displayedEvents,
      variants: [...new Set(displayedEvents.map(event => event.variant).filter(Boolean))],
    };
  });
}
...
function productIssueListHtml(events, selectedEventUid){
  ...
  return issueDisplayRows(events)
    ...
    .map(event => {
      if (event.grouped) {
        const groupedEvents = event.grouped_events || [];
        const pendingCount = groupedEvents.filter(item => !isEventHandled(item)).length;
        const variants = event.variants || [];
        const handled = pendingCount === 0;
        const selected = groupedEvents.some(item => item.event_uid === selectedEventUid);
        const basis = event.summary_reason || event.judge_basis?.['判定过程'] || event.judge_basis?.['命中依据'] || '系统暂未记录本次判定过程';
        const recommendation = event.recommendation || '建议暂不可用，请重新巡检后查看。';
        return `<div class="product-issue-row grouped-issue ${handled ? 'done-issue' : severityClass(event)} ${selected ? 'primary-issue' : ''}" role="button" tabindex="0" data-event-uid="${esc(event.event_uid)}">
          <span class="product-issue-cell product-issue-main"><span class="severity-badge ${severityClass(event)}">${esc(eventSeverity(event))}</span><span><b>${esc(event.issue || event.category || '异常')}</b><small>1 个业务异常 · ${variants.length || groupedEvents.length} 个子 ASIN 证据</small>${variants.length ? `<span class="product-issue-variants">${variants.map(variant => `<button type="button" data-event-focus="${esc((groupedEvents.find(item => item.variant === variant) || {}).event_uid || '')}">${esc(variant)}</button>`).join('')}</span>` : ''}</span></span>
          ...
          <button class="product-issue-status-button ${handled ? 'done' : 'pending'}" type="button" data-event-action="${esc(event.event_uid)}" data-event-toggle="${handled ? '重新打开' : '完成'}" title="${handled ? '点击重新打开整个业务异常' : '点击一次完成整个业务异常'}">${handled ? '已完成' : '待处理'} · ${variants.length || groupedEvents.length} 个子 ASIN</button>
        </div>`;
      }
      ...
      const basis = event.summary_reason || event.judge_basis?.['判定过程'] || event.judge_basis?.['命中依据'] || '系统暂未记录本次判定过程';
      const recommendation = event.recommendation || '建议暂不可用，请重新巡检后查看。';
      const autoRecovering = event.internal_status === '待观察' && event.last_action === '自动恢复观察';
      const scoreText = event.counts_toward_product_score === false
        ? `原 ${formatScore(event.score)} 分 · 已从当前执行分排除`
        : `${formatScore(event.score)} 分 · 已持续 ${event.days || 0} 天`;
      const statusControl = autoRecovering
        ? '<span class="product-issue-status-button done" title="本次巡检未再发现该异常，系统正在连续确认，无需运营处理。">疑似已恢复</span>'
        : `<button class="product-issue-status-button ${handled ? 'done' : 'pending'}" type="button" data-event-action="${esc(event.event_uid)}" data-event-toggle="${handled ? '重新打开' : '完成'}" title="${handled ? '点击重新打开异常' : '点击标记异常已完成'}">${esc(eventStatusLabel(event))}</button>`;
      return `<div class="product-issue-row ${handled ? 'done-issue' : severityClass(event)} ${selected ? 'primary-issue' : ''}" role="button" tabindex="0" data-event-uid="${esc(event.event_uid)}">
        <span class="product-issue-cell product-issue-main"><span class="severity-badge ${severityClass(event)}">${esc(eventSeverity(event))}</span><span><b>${esc(issue)}</b><small>${esc(scoreText)}</small>${event.variant ? `<small class="product-issue-variant">子 ASIN：${esc(event.variant)}</small>` : ''}</span></span>
        <span class="product-issue-cell product-issue-basis">${esc(basis)}</span>
        <span class="product-issue-cell product-issue-recommendation">${esc(recommendation)}</span>
        ${statusControl}
      </div>`;
...
// 选中的单个异常详情
function anomalyDetailHtml(e, productEvents = []){
  ...
  const isFiveBulletIssue = (e.issue || e.category) === '五点异常';
  const currentProductEvents = productEvents.filter(event =>
    !['已完成', '已关闭'].includes(event.status));
  const evidenceEvents = currentProductEvents.length ? currentProductEvents : productEvents;
  const variants = isFiveBulletIssue
    ? [...new Set(evidenceEvents.filter(event => (event.issue || event.category) === '五点异常').map(event => event.variant).filter(Boolean))]
    : (e.variant ? [e.variant] : []);
  const hitAsin = variants.length
    ? `<div class="anomaly-hit-asins"><div class="ab-section-label">命中子 ASIN</div><div class="anomaly-hit-asins-head"><span>${variants.length} 个命中子体${isFiveBulletIssue ? '（五点异常聚合展示）' : ''}</span>${variants.length > 1 ? `<button type="button" class="copy-asin-btn secondary" data-copy-variants="${esc(variants.join('\n'))}">复制全部</button>` : ''}</div><div class="anomaly-hit-asin-list">${variants.map(variant => `<span class="anomaly-hit-asin"><code>${esc(variant)}</code><button type="button" class="copy-asin-btn" data-copy-variant="${esc(variant)}">复制</button></span>`).join('')}</div></div>`
    : '';
  const recommendation = e.recommendation || '建议暂不可用，请重新巡检后查看。';
...
async function copyHitAsins(value){
...
function renderToday(){
  ...
  const remaining = Number(d.today_remaining_products ?? currentTask.total);
  ...
  const progressTotal = Number(d.today_progress_total_products ?? (remaining + done));
  document.getElementById('doneNum').textContent = done;
  document.getElementById('totalNum').textContent = progressTotal;
  document.getElementById('remainNum').textContent = remaining;
  ...
  document.getElementById('queueList').innerHTML = `
    ...
    <div class="queue-item ${state.filters.quickMode==='all'?'active':''}" data-queue="all"><i class="qdot green"></i><div class="qname">全部产品<small>含无异常产品</small></div><div class="qcount">${products.length}</div></div>
  `;
  ...
  const filteredProducts = sortProducts(products.filter(product => {
    const productSearchText = [
      product.parent_asin, product.product_name, product.parent_sku,
      ...product.events.map(event => `${event.parent_asin || ''}${event.product_name || ''}${event.parent_sku || ''}`),
    ].join('').toLowerCase();
  ...
  document.getElementById('productTotalLabel').textContent = `共 ${products.length} 个产品`;
  ...
  body.querySelectorAll('.product-task-card[data-product-index]').forEach(card => {
    card.onclick = event => {
      // 标题链接应直接打开 Amazon 前台，不能被卡片选择动作拦截。
      if (event.target.closest('.product-amazon-link')) return;
...
function selectProduct(key){
  ...
  if (!productEvents.length) {
    state.selectedEventUid = null;
    renderNormalProductDetail(product, sameProduct, previousScrollTop);
    return;
  }
  const main = productEvents[0];
  const productScore = product.product_score;
  const productStatus = product?.product_status || '未完成';
  const imageUrl = productEvents.find(item => item.image_url)?.image_url || '';
  const displayIssues = issueDisplayRows(productEvents);
  const severityCounts = ['S0', 'S1', 'S2'].map(severity => ({
    severity, count: displayIssues.filter(item => eventSeverity(item) === severity).length,
  ...
  document.getElementById('detailTitle').innerHTML = `
    <div class="product-header-main">
      ...
      <div class="product-header-facts">
        ...
        <span><small>异常</small><b>${displayIssues.length} 类</b></span>
  ...
  detailScroll.innerHTML = `
    <section class="summary-section product-issues-section">
      <div class="summary-section-head"><span>本产品异常</span><div class="summary-issues-tools"><small>${displayIssues.length} 个业务异常 · ${productEvents.length} 个命中证据 · ${severityCounts.map(item => `${item.severity} ${item.count}`).join(' · ')}</small><a class="btn ghost-blue ad-agent-summary-btn" href="${buildAdAgentUrl(adEvent)}" target="_blank" rel="noopener" onclick="markDoingAndOpen('${esc(adEvent.event_uid)}')">点击进入广告决策Agent页面</a></div></div>
    ...
    ${product.has_active_maintenance ? `<div class="maintenance-review-notice"><span>✓ ${Number(product.active_observation_count || 1)} 个业务异常正在效果观察</span><small>最近观察节点：${esc(product.observation_at || '-')} · 同类子 ASIN 合并复盘</small><button type="button" class="btn ghost-blue" data-open-review="${esc(product.key)}">查看效果观察</button></div>` : ''}
    ${anomalyDetailHtml(selectedEvent, productEvents)}
...
function renderNormalProductDetail(product, sameProduct, previousScrollTop){
  const detailScroll = document.getElementById('detailScroll');
  const inspectionTime = product.inspection_time ? String(product.inspection_time).slice(0, 16) : '未关联';
  document.getElementById('detailTitle').innerHTML = `
    <div class="product-header-main">
      <div class="product-header-media">${productImageHtml(product.image_url, 'detail-header-image')}</div>
      <div class="product-header-copy">
        <div class="product-header-title">${esc(product.product_name || product.parent_asin)}</div>
        <div class="product-header-meta">父 ASIN：${esc(product.parent_asin || '-')} · 店铺：${esc(product.shop_account || '-')} · ${esc(product.site || '-')}</div>
        <div class="product-header-submeta">父 SKU：${esc(product.parent_sku || '-')} · 巡检时间：${esc(inspectionTime)}</div>
      </div>
      <div class="product-header-facts">
        <span><small>执行分</small><b>${formatScore(product.product_score)}</b></span>
        <span><small>产品定位</small><b class="product-tier-value">${esc(product.tier_display || product.tier || '待补充')}</b></span>
        <span><small>异常</small><b>0 项</b></span>
        <span class="product-header-status"><small>产品状态</small><b class="status normal">正常</b></span>
      </div>
    </div>`;
  detailScroll.innerHTML = `<section class="summary-section product-issues-section"><div class="summary-section-head"><span>本次巡检</span></div><div class="detail-empty">本次巡检未发现异常，无需提交维护或进入效果观察。</div></section>`;
  detailScroll.scrollTop = sameProduct ? previousScrollTop : 0;
  document.querySelectorAll('.product-task-card').forEach(card => card.classList.toggle('selected', card.dataset.productKey === product.key));
  const completeProductBtn = document.getElementById('completeProductBtn');
  const aiBtn = document.getElementById('aiBtn');
  completeProductBtn.disabled = true;
  completeProductBtn.textContent = '本产品暂无待处理异常';
  aiBtn.disabled = true;
  renderDetailAssign(product);
}
...
function renderDetailAssign(product){
  ...
  const owners = activeErpOwners(product)
    .map(owner => esc(owner.user_name))
    .join('、') || '待 ERP 修正';
  const current = (product.assignments || []).map(a =>
    `<span class="assign-badge">👤 ${esc(a.assignee_name)}${a.can_revoke ? ` <a href="javascript:void(0)" data-revoke="${a.assignee_id}" title="撤回">✕</a>` : ''}</span>`
  ).join(' ');
  wrap.innerHTML = `
    ...
    <span class="detail-assign-current">ERP 负责人：${owners}</span>
    ${current ? `<span class="detail-assign-current">协作指派：${current}</span>` : ''}`;
...
async function loadSchedulePreview(eventUid, expectedAvailableAt = ''){
  ...
  try {
    ...
    const inspectionRow = document.getElementById('opInspectionRow');
    const reviewSelect = document.getElementById('opReview');
    const reviewCustomWrap = document.getElementById('opReviewCustomWrap');
    const reviewCustomDays = document.getElementById('opReviewCustom');
    const inspectionFollowReview = document.getElementById('opInspectionFollowReview');
    const inspectionSelect = document.getElementById('opInspection');
    const inspectionCustomWrap = document.getElementById('opInspectionCustomWrap');
    const manualAllowed = Boolean(schedule.allows_manual_schedule);
    if (option) option.textContent = `按规则：${schedule.rule_name} · ${schedule.days}天后`;
    if (hint) hint.textContent = `${schedule.source === 'expected_arrival' ? '按预计到货/上架日' : `默认规则：${schedule.rule_name}`} · ${formatIsoMonthDay(schedule.observation_at)}${manualAllowed ? '；可由运营调整' : '；此异常按规则自动复查'}`;
    if (expectedRow) expectedRow.style.display = schedule.supports_expected_available_date ? '' : 'none';
    if (reviewSelect) {
      reviewSelect.disabled = !manualAllowed;
      if (!manualAllowed) reviewSelect.value = 'default';
    }
    if (reviewCustomWrap && !manualAllowed) reviewCustomWrap.style.display = 'none';
    if (reviewCustomDays && !manualAllowed) reviewCustomDays.required = false;
    if (inspectionRow) inspectionRow.style.display = manualAllowed ? '' : 'none';
    if (!manualAllowed) {
      if (inspectionFollowReview) inspectionFollowReview.checked = true;
      if (inspectionSelect) inspectionSelect.disabled = true;
      if (inspectionCustomWrap) inspectionCustomWrap.style.display = 'none';
    }
  } catch (error) {
    if (state.selectedEventUid !== requestUid) return;
    const option = document.querySelector('#opReview option[value="default"]');
...
function applyTaskActionLocally(eventUid, result){
  ...
  if (Array.isArray(result.events)) {
    const refreshedByUid = new Map(result.events.map(item => [item.event_uid, item]));
    product.events.forEach(item => {
      const refreshed = refreshedByUid.get(item.event_uid);
      if (!refreshed) return;
      Object.assign(item, refreshed);
      const flatEvent = state.todayData?.events?.find(candidate => candidate.event_uid === item.event_uid);
      if (flatEvent && flatEvent !== item) Object.assign(flatEvent, refreshed);
    });
  } else {
...
function updateTodaySummaryLocally(){
  ...
  const remaining = Number(state.todayData?.today_remaining_products ?? currentTask.total);
  const done = Number(state.todayData?.done_today_products || 0);
  const progressTotal = Number(state.todayData?.today_progress_total_products ?? (remaining + done));
  const handledEvents = products.reduce((sum, product) => sum + Number(product.handled_event_count || 0), 0);
  const totalEvents = products.reduce((sum, product) => sum + Number(product.event_count || 0), 0);
  document.getElementById('doneNum').textContent = done;
  document.getElementById('totalNum').textContent = progressTotal;
  document.getElementById('remainNum').textContent = Math.max(0, remaining);
  document.getElementById('doneEventNum').textContent = handledEvents;
  document.getElementById('totalEventNum').textContent = totalEvents;
...
function bindAnomalyBlockEvents(){
  ...
  document.querySelectorAll('#detailScroll [data-event-focus]').forEach(control => {
    control.onclick = event => {
      event.stopPropagation();
      if (control.dataset.eventFocus) focusEvent(control.dataset.eventFocus);
    };
  });
  document.querySelectorAll('#detailScroll [data-copy-variant], #detailScroll [data-copy-variants]').forEach(control => {
    control.onclick = async event => {
      event.stopPropagation();
      const value = control.dataset.copyVariant || control.dataset.copyVariants;
      if (!value) return;
      try {
        await copyHitAsins(value);
        toast(control.dataset.copyVariants ? '已复制全部命中子 ASIN' : `已复制 ${value}`);
      } catch {
        toast('复制失败，请手动复制');
      }
    };
  });
  document.querySelectorAll('#detailScroll [data-open-review]').forEach(control => {
    control.onclick = () => {
      const product = state.todayData?.products?.find(item => item.key === control.dataset.openReview);
      if (!product) return;
      const selected = product.events.find(item => item.event_uid === state.selectedEventUid);
      state.reviewFocus = {
        parent_asin: product.parent_asin,
        shop_account: product.shop_account,
        issue: selected?.issue || selected?.category || '',
        event_uids: product.observing_event_uids || [],
      };
      switchView('review');
    };
  });
  ...
  try {
    ...
    const result = await api(`/api/tasks/${eventUid}/action`, {
      ...
      body: JSON.stringify({
        ...
        review_at: isComplete && isSelectedEvent ? selectedReviewDate() : undefined,
        next_inspection_at: isComplete && isSelectedEvent ? selectedInspectionDate() : undefined,
        expected_available_at: isComplete && isSelectedEvent ? document.getElementById('opExpectedAvailableAt')?.value || undefined : undefined,
        notes,
      })
    });
    const affectedCount = Number(result.changed_event_uids?.length || 1);
    const actionMessage = actionType === '完成'
      ? `该业务异常已完成，${affectedCount} 个子 ASIN 进入同一效果观察（${result.observation_at}）`
      : actionType === '重新打开' ? '已重新打开该业务异常' : actionType === '标记处理中' ? '已标记该业务异常为处理中' : '已记录不处理原因并关闭该业务异常';
    toast(actionMessage);
...
async function submitProductMaintenance(){
  ...
  try {
    ...
    toast(`已完成 ${result.completed_count} 个命中证据，并按业务异常合并创建效果观察`);
...
function buildAdAgentUrl(e){
  const q = new URLSearchParams({
    ...
    productPosition: e.product_position || '',
    productStage: e.product_stage || '',
    seasonType: e.season_type || '',
    siteCode: e.site || '',
